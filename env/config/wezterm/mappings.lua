...
M = {
	keys = {
		...
		{ key = 's', mods = 'LEADER|CTRL',      action = sessionizer.show(schemes.sessions), },
		...
		},
		{
			...
			action = wezterm.action_callback(function(window, pane)
				sessionizer.DefaultCallback(window, pane, sessions.work[1])
			end),
		},
		{
			key = 'I', mods = 'LEADER|SHIFT',
			action = wezterm.action_callback(function(window, pane)
				sessionizer.DefaultCallback(window, pane, sessions.work[2])
			end),
		},
		{
			key = 'O', mods = 'LEADER|SHIFT',
			action = wezterm.action_callback(function(window, pane)
				sessionizer.DefaultCallback(window, pane, sessions.work[3])
			end),
		},
		{
			key = 'P', mods = 'LEADER|SHIFT',
			action = wezterm.action_callback(function(window, pane)
				sessionizer.DefaultCallback(window, pane, sessions.work[4])
			end),
		},
	},
	key_tables = {
